...
impl VideoMemory for VRam {
    ...
    fn get_pattern_table_tile_data(&self, table_index: u8, tile_index: u8, mapper: &mut MapperEnum) -> [u8; 16] {
        ...
        for (i, tile_data) in tile_data.iter_mut().enumerate() {
            *tile_data =
                self.get_byte_internal(pattern_table_addr + 16 * tile_index as u16 + i as u16,mapper);
        }
        tile_data
    }
    ...
    fn get_attribute_data(&self, table_index: u8, color_tile_x: u8, color_tile_y: u8, mapper: &mut MapperEnum) -> u8 {
        let attrib_table_addr =
            self.get_target_address(NAMETABLES_START + table_index as u16 * NAMETABLE_SIZE + 960,mapper);
    ...
    fn get_low_pattern_data(&self, table_index: u8, tile_index: u8, y: u8,mapper: &mut MapperEnum) -> u8 {
        let pattern_table_addr = table_index as u16 * PATTERN_TABLE_SIZE;
        self.get_byte_internal(pattern_table_addr + 16 * tile_index as u16 + y as u16,mapper)
    }

    fn get_high_pattern_data(&self, table_index: u8, tile_index: u8, y: u8, mapper: &mut MapperEnum) -> u8 {
        let pattern_table_addr = table_index as u16 * PATTERN_TABLE_SIZE;
        self.get_byte_internal(pattern_table_addr + 16 * tile_index as u16 + 8 + y as u16,mapper)
    }
